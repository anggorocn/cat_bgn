...
class Asesor extends Model
{
    ...
    public function selectByParamsMonitoring($statement='',$reqUnitKerja='',$reqJenis='', $order='')
    {
      ...
      if(!empty($statement))
      {
          $query->whereRaw("A.user_app_id = ? ",[$statement]);
          // vuln sqli
    ...
    public function selectByParamsMonitoringAsesor($statement='',$reqUnitKerja='',$reqJenis='', $order='order by user_app_id desc')
    {
      $query="
           SELECT *,
           case when user_group_id = 1 then 'Administrator'
           when user_group_id = 2 then 'Asesor'
           end groub_nama from user_app
