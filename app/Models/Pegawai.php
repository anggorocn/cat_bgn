...
class Pegawai extends Model
{
    ...
    protected $table = 'simpeg.pegawai';
    ...
    public function selectByParamsMonitoring($statement='')
    {
      ...
      $str = "
      select
      a.*, b.nama eselon_nama, s.nama nama_satker
      FROM simpeg.pegawai A 
      left join eselon b on b.ESELON_ID = a.LAST_ESELON_ID
      left join simpeg.satker s on s.satker_id = a.satker_id
      WHERE 1 = 1 ".$statement." order by pegawai_id desc";
      // echo $str;exit;
    ...
    public function selectByParamsSatker($statement='')
    {
      // \DB::enableQueryLog();
      $str = "
      select
      a.* FROM simpeg.satker A 
      WHERE 1 = 1 ".$statement." order by satker_id asc";
      // echo $str;exit;
    ...
    function selectByParamsEselon( $order="ORDER BY cast(urutan as int) asc")
    { 
      $str = "
      select
      *
    ...
    function selectByParamsHome($statement="", $order="ORDER BY A.TANGGAL_TES DESC")
    { 
      $str = "
      SELECT
      b.jadwal_awal_tes_id, a.acara, tanggal_tes, tanggal_tes_akhir, count(jadwal_awal_tes_simulasi_pegawai_id) terdaftar
      FROM
      jadwal_awal_tes A 
      LEFT JOIN jadwal_awal_tes_pegawai b ON A.jadwal_awal_tes_id = b.jadwal_awal_tes_id 
      LEFT JOIN jadwal_awal_tes_simulasi_pegawai c ON A.jadwal_awal_tes_id = c.jadwal_awal_tes_id and b.pegawai_id=c.pegawai_id
      WHERE 1 = 1 
      "; 

      $str .= $statement." 
      GROUP BY a.acara, tanggal_tes, tanggal_tes_akhir,b.jadwal_awal_tes_id
      ".$order;
    ...
    function selectByParamsJadwalAwalTesSimulasi($statement="",$pegawai_id="", $order="ORDER BY TANGGAL_tes DESC")
    { 
      $str = "
      SELECT a.jadwal_awal_tes_simulasi_id, acara , tanggal_tes, count(b.jadwal_awal_tes_simulasi_pegawai_id) terdaftar,count(c.jadwal_awal_tes_simulasi_pegawai_id) total_terdaftar, batas_pegawai
      FROM jadwal_awal_tes_simulasi a 
      LEFT JOIN jadwal_awal_tes_simulasi_pegawai b ON A.jadwal_awal_tes_simulasi_id = b.jadwal_awal_tes_simulasi_id  and b.pegawai_id=".$pegawai_id."
