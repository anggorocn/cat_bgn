    ...
    public function index()
    {
        ...
        // Mengecek state yang disimpan saat ini untuk memitigasi serangan CSRF
            ...
            $token = $provider->getAccessToken('authorization_code', [
                    'code' => $_GET['code'],
                    'redirect_uri' => 'http://localhost:8000/', // Harus sama persis!
                ]);
            // try {
            //     $token = $provider->getAccessToken('authorization_code', [
            //         'code' => $_GET['code']
            ...
            try {
                ...
                $nipcek= $user->getNipBaru();
                // print_r($nipcek);exit;
                $user = UserApp::where('user_login', $nipcek)->first();
                ...
                else
                {
                    return Redirect::back()->withErrors(
                        [
                            'login_gagal' => 'Login SSO berhasil, User tidak ditemukan di Aplikasi Assesment'
                        ]
                    );
                }
