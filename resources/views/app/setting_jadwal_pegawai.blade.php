                                                ...
                                                <div class="card-header row" style="margin: 10px;">
                                                    <div class="card-title" style="margin-bottom: 0rem;width: 75%;">
                                                        <div class="row">
                                                            ...
                                                            <h3 class="card-label" style="color:black;width: 50%;margin-left: 25px;">List Asesor</h3>
                                                        </div>
                                                    </div>  
                                                </div>
                                                <div class="card-body" style="height:45vh;overflow: scroll;">
                                                    <div class="form-group row">
                                                        ...
                                                        <div class="row" style="width:100%">
                                                            <button type="button" class="btn btn-warning font-weight-bold mr-2" style="margin: 10px;width: 25%;background-color: #28a745;" disabled><h3></h3></button> 
                                                            <span style="padding:15px">= Kuota Penuh</span>
                                                        </div>
                                                        <table id="customers">
                                                            <tr>
                                                                <th>Nama Asesor</th>
                                                            ...
                                                            for($i=0; $i<count($queryhead);$i++){
                                                                $bg='';
                                                                if($queryhead[$i]['batas_pegawai']==$queryhead[$i]['total_terpilih']){
                                                                    $bg='background-color: #28a745';
                                                                }
                                                                ?>
                                                                <tr style="<?=$bg?>">
                                                                    <td>
                                                                        <?=$queryhead[$i]['nama_asesor']?>
                                                                    </td>
                                                                    <td>
                                                                        <?=$queryhead[$i]['nama_penggalian']?> 
                                                                        (<?=$queryhead[$i]['kode']?>)
                                                                    </td>
                                                                    <td> 
                                                                        <?=$queryhead[$i]['batas_pegawai']?>
                                                                    </td>
                                                                    <td style='margin 10px'>
                                                                        <?php 
                                                                        if($queryhead[$i]['status_group']==1){
                                                                            $link='app/setting_jadwal/pegawaidetilkelompok/'.$reqId.'/'.$queryhead[$i]["jadwal_asesor_id"].'/'.$queryhead[$i]["penggalian_id"];
                                                                        }
                                                                        else{
                                                                            $link='app/setting_jadwal/pegawaidetil/'.$reqId.'/'.$queryhead[$i]["jadwal_asesor_id"].'/'.$queryhead[$i]["penggalian_id"];
                                                                        }
                                                                        if($reqDetilId!=$queryhead[$i]['jadwal_acara_id']){?> 
                                                                            <a href='<?=$link?>' class="btn btn-warning font-weight-bold mr-2"><i class="fa fa-edit"></i></a>
                                                                        <?php }?>
                                                                    </td>
                                                                </tr>
                                                            <?php }?>
...
// menu samping
        ...
        var url = "SettingJadwal/AddPegawai";
        ...
        {
            openAdd('/app/setting_jadwal/lookup/pegawai/<?=$reqId?>/<?=$reqPenggalianId?>');
        }
                ...
                if (value === undefined) {
                    ...
                    $('.tambahsini tbody').append(`
                        <tr>
                            ...
                            <input type='hidden' value='' name='reqJadwalPegawaiId[]'>
                            </td>
                            <td>`+value.nip+`</td>
        ...
        function deletedata(id) {
            ...
            {
                urlAjax= "/SettingJadwal/deletePegawai/"+id;
                swal.fire({
                        ...
                        $.ajax({
                            ...
                            success : function(data) { 
                                swal.fire({
                                    ...
                                    document.location.href = "app/setting_jadwal/pegawai/<?=$reqId?>/<?=$reqDetilId?>/<?=$reqPenggalianId?>";
                                });
                            },
                            complete: function() {
                                swal.hideLoading();
