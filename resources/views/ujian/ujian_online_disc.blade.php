                            ...
                            <div class="card-body" style="height: 55vh;">
                                <section class="your-class">
                                    <?php
                                        ...
                                        <div>
                                            <div class="row">
                                                <div class="col-lg-12"><h4> <?=$nomer?>. </h4></div> 
                                                <?php $nomer++;?>

                                                <div class="col-lg-1" style="text-align: right;"><h2>+</h2></div>
                                                ...
                                                <div class="col-lg-1">
                                                    <div class="row">
                                                        ...
                                                        for($j=0; $j<count($arrayJawaban);$j++){
                                                            ...
                                                            $jawaban=$queryJawaban[$arrayJawaban[$j]]['jawaban'];
                                                        ...
                                                        ?>
                                                            <div class="col-lg-12" style="text-align:right;">
                                                                <h4><input type="radio" <?=$checked?> id='radio-<?=$bank_soal_id?>-<?=$j?>-<?=$bank_soal_pilihan_id?>' name='radio-<?=$bank_soal_id?>'></h4>
                                                            </div>
                                                        <?php }?>
                                                    </div>
                                                </div>
                               ...
                               <a class="btn btn-warning font-weight-bold mr-2" style="width: 90%; margin:10px 5%;display:none" id='selesai' onclick="setFinish()"><h3>Selesai</h3></a>
                                <div class="card-body" style="height: 60vh; overflow: scroll;">
                                    <div class="row">
                                        <?php 
                                        $totalterjawab=0;
                                        for($i=0; $i<count($querySoal)/2;$i++){
                                            $classbtn='btn-success';
                                            $idkanan=($i*2)+1;
                                            $idkiri=$idkanan-1;
                                            $bank_soal_id_kanan=$querySoal[$idkanan]['bank_soal_id'];
                                            $bank_soal_id_kiri=$querySoal[$idkiri]['bank_soal_id'];
                                            if(!empty($queryJawabanPeserta)){

                                                $arrayJawabanPeserta_kanan=StringFunc::in_array_column($bank_soal_id_kanan, 'bank_soal_id', $queryJawabanPeserta);
                                                $arrayJawabanPeserta_kiri=StringFunc::in_array_column($bank_soal_id_kiri, 'bank_soal_id', $queryJawabanPeserta);
                                                if(count($arrayJawabanPeserta_kanan)!=0 && count($arrayJawabanPeserta_kiri)!=0){
    ...
    <script type="text/javascript">
        ...
        $(document).on('ready', function() {
            ...
            $('input[id^="radio"]').change(function(e) {
                ...
                argument= tempId.split('radio-');
                argument= argument[1].split('-');
                soalid=argument[0]
                dataid=argument[1]
                jawabanid=argument[2]
                ceksoal=parseInt(soalid);
                ...
                const checkbox =  $('input[id^="radio-'+ceksoal+'-'+dataid+'"]');
                
                if (checkbox.prop('checked')) {
                    var checkboxid= $(this).attr('id');

                    alert("Data Kiri dan Kanan Tidak Boleh Sama");
                ...
                const checkedCount = $('input[id^="radio-'+ceksoal+'-"]:checked').length;
                if(checkedCount==1){
                    if( soalid%2 == 1){
                       ceksoal=ceksoal
                    }
                    ...
                    $("#nomor-"+gas).removeClass("btn-success").addClass("btn-warning");
        ...
        $('#calculateTotal').click(function() {
            ...
            // console.log(<?=count($querySoal)/2?>)
            if((total-4)==<?=count($querySoal)/2?>){
                $('#selesai').show();
            }
        });  
        ...
        function terjawab(argument) { 
            ...
            tempBankSoalPilihanId= tempId[2];
            // catatan reqId/reqUjianId/reqTipeUjianId/reqBankSoalId/reqBankSoalPilihanId/reqPegawaiId
            var s_url= "Ujian/jawab/<?=$reqId?>/<?=$user->ujian_id?>/<?=$queryIdentitas->tipe_ujian_id?>/"+tempBankSoalId+"/"+tempBankSoalPilihanId+"/<?=$user->pegawai_id?>";
            $.ajax({'url': s_url,'success': function(msg) {
                if(msg == '')
                {
